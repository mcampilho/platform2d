...
class PrecisionScene:
    ...
    def update(self,dt,actions):
        ...
        self.player.update(dt,actions,self.level.colliders,self.platforms,ladders=self.ladders)
    ...
    def draw_status_overlay(self, surface, lessons):
        ...
        image = self.large.render(("Percurso concluído" if lessons else "Cume alcançado") if self.won else "Em pausa",True,(219,247,233))
    ...
    def draw_hud(self,surface,lessons):
        ...
        self.text(surface,self.level.properties.get("lab_title","PLATFORM2D / LABORATÓRIO DE MOVIMENTO") if lessons else "PLATFORM2D / LABORATÓRIO 04",25,12,(126,236,215))
        self.text(surface,self.level.name,24,33,(231,242,249),self.title)
        if lessons:
            lesson = max((item for item in lessons if self.player.body.x >= item["x"]),
                         key=lambda item:item["x"],default=lessons[0])
            self.text(surface,lesson["title"],245,32,(126,236,215))
            self.text(surface,lesson["help"],245,54,(177,201,220))
        else:
        ...
        self.text(surface,(f"ZONA {lessons.index(lesson)+1}/{len(lessons)}" if lessons else
                  f"SINAIS {len(self.collected)}/{self.total}"),794,23,(247,200,111))
        self.text(surface,f"TENTATIVAS {self.deaths+1:02}",794,47)
        pygame.draw.rect(surface,(8,15,28),(0,544,960,32))
        self.text(surface,self.level.properties.get("footer","A D mover   ESPAÇO salto   SHIFT/C dash   X planar   R checkpoint   F1 debug   P pausa") if lessons else
                  "A D  mover   W S  escada   ESPAÇO  salto   SHIFT / C  dash   R  checkpoint   F1  debug   P  pausa",18,555)

    def draw(self,surface,alpha):
        cx,cy = self.camera.interpolated(alpha)
        offset=getattr(self,"camera_offset",None)
        if offset is not None:
            ox,oy=offset(alpha); cx+=ox; cy+=oy
        if hasattr(self,"draw_background"):
            self.draw_background(surface,cx,cy)
        else:
        ...
        if hasattr(self,"draw_foreground"):
            self.draw_foreground(surface,cx,cy)
        ...
        for c in self.level.colliders:
            ...
            if x+32 < 0 or x > 960:
                continue
            if hasattr(self,"draw_terrain"):
                self.draw_terrain(surface,c,x,y)
            elif c.slope:
        ...
        for obj in self.level.objects:
            ...
            if hasattr(self,"draw_world_object") and self.draw_world_object(surface,obj,x,y):
                continue
            if obj["type"] == "beacon":
                ...
                self.text(surface,"FIM" if self.level.properties.get("lessons") else "CUME",x-1,y-23,color)
        ...
        if hasattr(self,"draw_environment"):
            self.draw_environment(surface,alpha,cx,cy)
        if hasattr(self,"draw_player"):
            self.draw_player(surface,alpha,cx,cy)
        else:
            x,y = self.player.body.interpolated(alpha)
            self.view.draw(surface,(x-cx-4,y-cy-10),self.player.controller.facing)
        if hasattr(self,"draw_environment_effects"):
            self.draw_environment_effects(surface,alpha,cx,cy)
        ...
        self.draw_hud(surface, lessons)
        if self.paused or self.won:
            self.draw_status_overlay(surface, lessons)
