"""Run the mobility laboratory: python -m examples.mobility."""
import argparse
import json
import os
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Laboratório de mobilidade Platform2D")
    ...
    try:
        ...
        game = Game(scene, settings["bindings"], size=(960, 576),
                    title="Platform2D — Laboratório de Mobilidade", volume=args.volume,
                    muted=args.mute or args.headless, controls_profile="precision",
                    controls_path=args.controls_dir / "mobility.controls.json")
        if args.screenshot:
